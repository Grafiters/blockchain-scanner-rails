...
describe 'job.rake' do
  context "Rake::Task['job:liabilities:compact_orders']" do
    ...
    it 'call rake task with default time range', clean_database_with_truncation: true do
      ...
      result = ActiveRecord::Base.connection.query("SELECT NULL, code, currency_code, member_id, SUM(debit), SUM(credit) FROM liabilities WHERE (LOWER(reference_type) = LOWER('Order') AND created_at BETWEEN '#{min}' AND '#{max}') GROUP BY code, member_id, currency_code, DATE(created_at)")
      subject.invoke
