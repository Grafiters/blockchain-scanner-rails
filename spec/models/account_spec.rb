...
describe Account do
  ...
  describe 'concurrent lock_funds' do
    it 'should raise error on the second lock_funds' do
      ...
      end.to raise_error(Account::AccountError) { |e| expect(e.message).to eq "Cannot lock funds (member id: #{subject.member_id}, currency id: #{subject.currency_code}, amount: 8, balance: 2.0, locked: 18.0)." }
